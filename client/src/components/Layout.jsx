...
const navGroups = [
  { label: 'Workspace', links: [['/', 'Dashboard'], ['/expenses', 'Expenses'], ['/accounts', 'Accounts']] },
  { label: 'Insights', links: [['/analytics', 'Analytics']] },
...
export default function Layout({ user, theme, setTheme, onLogout, hideAmounts, onToggleAmounts, children }) {
  ...
  return <div className={`app-shell ${hideAmounts ? 'amounts-hidden' : ''}`}>
    ...
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav">{navGroups.map(group => <div className="sidebar-nav-group" key={group.label}><span className="sidebar-label">{group.label}</span>{group.links.map(([to, label]) => <NavLink key={to} to={to} end={to === '/'} title={label} aria-label={label} className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><span className="nav-icon">{navIcons[label]}</span><span className="nav-link-label">{label}</span></NavLink>)}</div>)}</nav>
