...
function Donut({ categories, total, currency, hidden }) {
  ...
  return <div className="analytics-donut-wrap"><div className="analytics-donut-chart"><ResponsiveContainer width="100%" height={190} initialDimension={{ width: 190, height: 190 }}><RechartsPieChart><Tooltip position={{ x: 12, y: 8 }} allowEscapeViewBox={{ x: false, y: false }} formatter={value => [hidden ? '****' : money(value, currency), 'Spent']} contentStyle={{ border: '1px solid var(--line)', borderRadius: 8, background: 'var(--surface)', color: 'var(--ink)' }} /><Pie isAnimationActive={false} data={segments} dataKey="amount" nameKey="category" startAngle={-270} endAngle={-630} innerRadius={52} outerRadius={78} paddingAngle={1} stroke="var(--surface)" strokeWidth={2}>{segments.map(item => <Cell key={item.category} fill={item.color} />)}</Pie></RechartsPieChart></ResponsiveContainer><div className="analytics-donut-center"><strong>{hidden ? '****' : shortMoney(total, currency)}</strong><span>Total spent</span></div></div><div className="analytics-legend">{segments.map(item => <div className="analytics-legend-item" key={item.category}><i style={{ background: item.color }} /><span>{item.category}</span><strong>{item.percent.toFixed(0)}%</strong></div>)}</div></div>
}
