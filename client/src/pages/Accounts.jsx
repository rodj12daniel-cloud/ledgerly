...
export default function Accounts({ user, hideAmounts }) {
  ...
  return <><header className="page-header"><div><span className="eyebrow">Accounts and sources</span><h1>Your accounts</h1><p className="muted">Manual balances for the places you keep money.</p></div><LedgerlyActionButton onClick={() => setEditing({ new: true })}>Add account</LedgerlyActionButton></header>{error && <div className="alert error">{error}</div>}<section className="accounts-summary"><span>Total across accounts</span><strong>{money(total, user.currency, hideAmounts)}</strong></section>{wallets.length === 0 ? <section className="wallet-empty page-empty"><strong>No accounts yet</strong><span>Add a manual source such as BPI, GCash, Cash, or Savings.</span><LedgerlyActionButton secondary onClick={() => setEditing({ new: true })}>Add an account</LedgerlyActionButton></section> : <section className="wallet-grid accounts-grid">{wallets.map(wallet => <WalletCard key={wallet._id} wallet={wallet} user={user} balance={convert(wallet.balance, wallet.currency || 'PHP')} hideAmount={hideAmounts} onEdit={() => setEditing(wallet)} onRemove={() => remove(wallet._id)} onColorChange={color => changeColor(wallet, color)} />)}</section>}{editing && <div className="modal-backdrop"><div className="modal"><WalletForm wallet={editing.new ? null : editing} userCurrency={user.currency} onSubmit={save} onCancel={() => setEditing(null)} /></div></div>}</>
}
