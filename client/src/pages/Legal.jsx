...
export default function Legal({ type, user }) {
  ...
  return <div className="legal-page"><div className="legal-top"><Link className="brand" to={returnPath}><img src="/assets/images/Black%20and%20Gold%20Elegant%20Banking%20Finance%20Logo%20(1).png" alt="Ledgerly" /></Link><Link className="legal-return-button" to={returnPath}>{returnLabel}</Link></div><main className="legal-card"><span className="eyebrow">{page.eyebrow}</span><h1>{page.title}</h1><p className="legal-intro">{page.intro}</p><div className="legal-sections">{page.items.map(([heading, copy]) => <section key={heading}><h2>{heading}</h2><p>{copy}</p></section>)}</div><p className="legal-footer">Questions about your data? Review the Settings page or delete your account from your profile.</p></main></div>
}
