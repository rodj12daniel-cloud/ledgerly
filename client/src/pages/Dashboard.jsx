...
export default function Dashboard({ user, theme, onThemeChange, hideAmounts }) {
  ...
  const [categoryChart, setCategoryChart] = useState('bar')
  ...
  const totalCategorySpending = categories.reduce((sum, [, amount]) => sum + amount, 0)
  const categorySlices = categories.reduce((slices, [category, amount], index) => {
    const start = slices.at(-1)?.end || 0
    return [...slices, { category, amount, index, start, end: start + (totalCategorySpending ? amount / totalCategorySpending * 100 : 0) }]
  }, [])
  const categoryGradient = categorySlices.map(slice => `${colors[slice.index % colors.length]} ${slice.start}% ${slice.end}%`).join(', ')
  ...
  return <>
    <header className="page-header">
      ...
      <div className="header-actions"><ThemePicker theme={theme} onChange={onThemeChange} /></div>
    </header>
    ...
    <section className="dashboard-grid"><div className="panel"><div className="panel-heading"><div><h2>Recent expenses</h2><p className="muted">Your latest activity</p></div><Link to="/expenses">View all</Link></div>{expenses.length === 0 ? <div className="empty-state"><span className="empty-illustration" aria-hidden="true"><span>+</span></span><strong>Your spending story starts here.</strong><span>Add your first expense and Ledgerly will keep the picture clear.</span><DashboardButton theme={theme} secondary onClick={() => navigate('/expenses?add=true')}>Add first expense</DashboardButton></div> : <div className="expense-list">{expenses.slice(0, 5).map(item => <div className="expense-row" key={item._id}><span className="category-icon"><CategoryIcon category={item.category} /></span><span className="expense-info"><strong>{item.description}</strong><small>{item.category} · {item.walletId?.label || 'No account selected'} · {new Date(item.date).toLocaleDateString()}</small></span><strong>{hideAmounts ? '****' : money(item.amount, user.currency)}</strong></div>)}</div>}</div><div className="panel"><div className="panel-heading"><div><h2>By category</h2><p className="muted">Where your money goes</p></div><div className="chart-toggle" role="group" aria-label="Chart type"><button type="button" className={categoryChart === 'bar' ? 'active' : ''} onClick={() => setCategoryChart('bar')}>Bar</button><button type="button" className={categoryChart === 'pie' ? 'active' : ''} onClick={() => setCategoryChart('pie')}>Pie</button></div></div>{categories.length === 0 ? <div className="empty-state compact"><span className="empty-illustration small" aria-hidden="true"><span>+</span></span><strong>Your categories will grow here.</strong><span>Add expenses to see your patterns.</span></div> : categoryChart === 'bar' ? <div className="category-list">{categorySlices.map(slice => <div className="category-item" key={slice.category}><div className="category-top"><span><i style={{ background: colors[slice.index % colors.length] }} />{slice.category}</span><strong>{money(slice.amount, user.currency, hideAmounts)}</strong></div><div className="bar"><span style={{ width: `${slice.end - slice.start}%`, background: colors[slice.index % colors.length] }} /></div></div>)}</div> : <div className="category-pie-layout"><div className="category-pie" style={{ background: `conic-gradient(${categoryGradient})` }} aria-label="Spending by category pie chart" /><div className="category-legend">{categorySlices.map(slice => <div className="category-legend-item" key={slice.category}><span><i style={{ background: colors[slice.index % colors.length] }} />{slice.category}</span><strong>{hideAmounts ? '****' : `${Math.round(slice.end - slice.start)}%`}</strong></div>)}</div></div>}</div></section>
